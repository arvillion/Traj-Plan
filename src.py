...
def plot_game(ctps_inter, target_pos, target_cls, class_scores, final_score):
    knots = torch.cat([
        torch.zeros(P),
    ...
    plt.title(f"Score={final_score}")
    plt.scatter(*ctps.T, color="red", label="control points")
    plt.scatter(*target_pos.T, c=class_scores[target_cls], alpha=.12, s=points_radius**2)
    scatter = plt.scatter(*target_pos.T, c=class_scores[target_cls], marker="*", label="targets")


    plt.legend(*scatter.legend_elements(), loc="lower left", title="Values", framealpha=.3)
